    ...
    public CategoryPageResponseDto getCategoryPageDetails(Long categoryId) {
        ...
        List<Course> mostSellingCourses = allCourses.stream()
                ...
                .sorted(Comparator.comparingInt(
                        (Course course) -> course.getStudentsEnrolled() != null ? course.getStudentsEnrolled().size()
                                : 0)
                        .reversed())
                .limit(10)
                .collect(Collectors.toList());

        CategoryPageResponseDto response = new CategoryPageResponseDto(selectedCategory, differentCategory,
                mostSellingCourses);
        return response;
    }
